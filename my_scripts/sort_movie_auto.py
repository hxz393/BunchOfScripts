...
from sort_movie_director import sort_director_auto
from sort_movie_mysql import insert_movie_wanted
from sort_movie_ops import get_ids, get_files_with_extensions, get_subdirs, parse_jason_file_name, delete_trash_files, check_local_torrent, move_all_files_to_root
from sort_movie_request import get_tmdb_search_response, get_douban_response, get_douban_search_details
...
def achieve_director(path: str) -> None:
    ...
    result = get_tmdb_director_movies_all(path, skip_existing=True)
    if not result:
        return
