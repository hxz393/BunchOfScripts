...
def load_scrapy_dhd(config: dict | None = None):
    ...
    ``torrent_to_magnet``，所以这里先注入假的依赖，避免测试依赖本地真实环境。
    """
    ...
    fake_torrent_module = types.ModuleType("torrent_to_magnet")
    fake_torrent_module.torrent_to_magnet = lambda _torrent_path: "magnet:?xt=urn:btih:fake"
    ...
    with patch.dict(
        sys.modules,
        {
            ...
            "torrent_to_magnet": fake_torrent_module,
        },
    ):
